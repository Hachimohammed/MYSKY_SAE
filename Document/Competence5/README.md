# GESTION DE PROJET DE LA SAE

**Diagramme de GANTT :** https://sharing.clickup.com/90152272888/g/h/2kyqx0zr-635/0da42b6273e2582

Initiales à coté de chaque tâche sur GANTT :  
YB = Yunes BOUABTA; S = Sofiane SEBBAH ; YS = Yakouba SOW ; K = Kenzo FILLONEAU ; HM = Mohammed Hachim

**Feuille de temps :**

**User Story _(fichier user_story_myskyv2.pdf)_ :** https://drive.google.com/file/d/1vkE6ZSjyA1sQio2L6jBvc8QlL9SQUdk7/view?usp=drivesdk

**Cahier des charges _(fichier MYSKY_CDG officiel.pdf)_ :** https://drive.google.com/file/d/1rBGbDXWnH-gNeEay7HxnTgvt8sOQxZMY/view?usp=sharing

---

## Présentation des maquettes

Conformément à votre demande, nous avons réalisé les **maquettes** et les avons rendues sous forme de **screens**, comme indiqué dans les consignes.

En complément, nous avons pris l’initiative de réaliser une **vidéo de présentation** afin de montrer l’**interactivité des maquettes** et de mieux illustrer les parcours utilisateurs.

La **première version des maquettes**, accompagnée de cette vidéo, a été envoyée au client du projet, **M. BRINGUIER**.  
Suite à ses **retours, modifications et conseils**, nous avons apporté les améliorations demandées et réalisé une **deuxième version des maquettes**.

## Contenu du répertoire `maquettes`

- Deux fichiers **ZIP** correspondant aux **deux versions des maquettes**

## Vidéo de présentation

Le **lien de la vidéo** réalisée pour la **première version des maquettes** est ajouté **directement dans ce message**.

🔗 [Voir la vidéo de présentation – Version 1] :    (https://www.youtube.com/watch?v=FU0ri1P4wrA)


---
